...
const { del, warn, hasPermissions } = require('./funct.js');
const { MessageEmbed } = require('discord.js');

let cooldown = new Set();

module.exports = {
    dbSetup: function (bot) {
        let guildsID = (bot.guilds.cache.map(guild => guild.id));
        let guildsName = (bot.guilds.cache.map(guild => guild.name));
        let commands = (bot.commands.map(cmd => cmd.name));
    ...
    messageXP: function (message, bot) {
        let guildID = message.guild.id;

        db.findOne({ guildID: guildID }, async (err, exists) => {
            if (!exists) return module.exports.dbSetup(bot);
            if (exists.xpSystem) {
                let guildName = message.guild.name;
                let userID = message.member.id;
                let userName = message.author.username;
                let xpToAdd = Math.floor(Math.random() * 4) + 1;
                ...
                db.findOne({ guildID: guildID }, (err, exists) => {
                    if (exists) {
                        ...
                        db.findOne({ guildID: guildID, channels: { $elemMatch: { command: "rank" } } }, (err, exists) => {
                            if (exists)
                                rankChannel = bot.channels.cache.get(exists.channels[exists.channels.map(cmd => cmd.command).indexOf("rank")].channelID);
                        }).catch(err => console.log(err))
                    } else {
