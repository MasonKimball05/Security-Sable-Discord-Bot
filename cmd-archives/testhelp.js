...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (message.channel.type === "dm") return message.channel.send(`This command can only be used in my test server`);
    if (message.guild.id !== "931626981047545946") {
      return message.reply(`sorry this command is a test server only command it can only be used in my test server!`)
    }
