      ...
      warns.findOne({
        ...
        async (err, data) => {
          ...
          if (!data) {
            ...
            bot.channels.cache.get(tsmodlog).send(`${user.tag} has been warned with the reason of **${args
              .slice(1)
              .join(" ")}**. They know have **${data.Warns.length}** warns.`);
            if (!modlog) return message.channel.send(
              `${user.name} has been warned with the reason of **${args.slice(1).join(" ")}**. They know have ${data.Warns.length} warns.`);
            try {
