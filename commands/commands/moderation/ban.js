            ...
            await message.channel.send.send(promptEmbed).then(async msg => {
                ...
                if (emoji === "✅") {
                    ...
                    toBan.ban(args.slice(1).join(" "))
                        .catch(err => {
                                if (err) return bot.channels.cache.get(modlog).send(`The ban didn't work! Here's the error: ${err}`)
                                if (!modlog) return message.reply(`The ban didn't work! Here's the error: ${err}`)
                        });
                    ...
                    toBan.send(`You have been banned from ${message.guild.name} \n\nReason: **${args.slice(1).join(" ")}`)
                        ...
                        message.channel.send(`Error occurred! I could not dm ${toBan}!`)
                } else if (emoji === "❌") {
                    msg.delete()
                        .then(() => {
                                bot.channels.cache.get(modlog).send(no);
                                bot.channels.cache.get(tsmodlog).send(no)
