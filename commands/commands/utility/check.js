...
module.exports = {
  ...
  accessableby: "Owner",
  aliases: ["age-check", "check-age", "ack"],
  usage: " <message id> <author id> <older/younger>",
    ...
    if (args[2] === 'older') {
      ...
      try {
        ...
        */
        ...
        const replyEmbed = new MessageEmbed()
          ...
          .setFooter(`${member.user.tag}'s UID: ${member.id}`)
          .setTimestamp();
        ...
        message.channel.send(replyo).then(() => bot.channels.cache.get(modlog).send(comp))
        bot.channels.cache.get(tsmodlog).send(comp)
        member.roles.add(verified)
        member.roles.remove(youngerr)
      ...
      try {
        ...
        */
        ...
        message.channel.send(replyy).then(() => bot.channels.cache.get(modlog).send(comp2))
        bot.channels.cache.get(tsmodlog).send(comp2)
        member.roles.add(verified)
        member.roles.remove(youngerr)
